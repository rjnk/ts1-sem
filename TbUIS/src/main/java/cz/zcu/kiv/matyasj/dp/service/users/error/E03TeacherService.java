 ...
 * @author Jakub Smaus
 */
public class E03TeacherService extends BaseTeacherService {
    ...
    /**
     * E03TeacherService constructor is needed because of dependencies in parent (BaseTeacherService) constructor.
     *
     ...
     */
    @Autowired
    public E03TeacherService(SubjectDao subjectDao, UserDao userDao, ExaminationDateDao examinationDateDao, GradeDao gradeDao, GradeTypeDao gradeTypeDao, DateUtility dateUtility, PropertyLoader propertyLoader) {
        super(subjectDao, userDao, examinationDateDao, gradeDao, gradeTypeDao, dateUtility, propertyLoader);
    }

    /**
     * This error method returns list of all Examination terms created by specific teacher.
    ...
    public List<ExaminationDate> getMyExaminationDatesWithoutGraduateParticipants(Teacher teacher) {
        ...
        List<ExaminationDate> dbExamTerms = new ArrayList<>(examinationDateList);

        if (!examTerms.isEmpty()) {
            for (ExaminationDate exam : dbExamTerms) {
                for (ExaminationDate e : examTerms) {
                    if (exam.getId().equals(e.getId())) {
                        examinationDateList.remove(exam);
                    }
                }
            }
        }
     ...
     * from view, but not from database
     *
     ...
     */
    @Override
    @ErrorMethod(errorMessage = "This error method removes examination term from view, but not from database.")
    public boolean removeExaminationTerm(Teacher teacher, Long examTermId) {
        ...
        log.error(propertyLoader.getProperty("log.E03TeacherService.removeExaminationTerm"));
